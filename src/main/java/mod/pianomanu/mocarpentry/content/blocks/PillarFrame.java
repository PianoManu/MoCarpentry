...
public class PillarFrame extends mod.pianomanu.blockcarpentry.block.FrameBlock implements IWaterLoggable {
    ...
    public PillarFrame(Properties properties) {
        super(properties);
        this.setDefaultState(this.stateContainer.getBaseState().with(CONNECTED_UP, false).with(CONNECTED_DOWN, false).with(SIZE, PillarSize.SMALL).with(SIDE, Direction.NORTH).with(WATERLOGGED, Boolean.valueOf(false)).with(BCBlockStateProperties.CONTAINS_BLOCK, false).with(LIGHT_LEVEL, 0));
    }
    ...
    public VoxelShape getShape(BlockState state, IBlockReader worldIn, BlockPos pos, ISelectionContext context) {
        if (state.get(SIZE) == PillarSize.SMALL) {
            ...
            } else if (state.get(CONNECTED_DOWN)) {
                if (state.get(CONNECTED_UP)) {
                    return FULL_SPM_SHAPE;
                } else {
    ...
    protected void fillStateContainer(StateContainer.Builder<Block, BlockState> builder) {
        builder.add(CONNECTED_UP, CONNECTED_DOWN, SIZE, SIDE, WATERLOGGED, BCBlockStateProperties.CONTAINS_BLOCK, LIGHT_LEVEL);
    }
